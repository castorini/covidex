...
class Highlighter:
    def __init__(self):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(
            settings.highlight_model, do_lower_case=False
        )
        print("Loading model...")
        self.model = transformers.AutoModel.from_pretrained(settings.highlight_model)
    ...
    def text_to_vectors(self, text: str):
        ...
        text_ids = torch.tensor(self.tokenizer.encode(text, add_special_tokens=True))
    ...
    def adjust_highlights(self, original_text: str, highlighted_text: str):
        ...
        while original_idx < len(original_text) and hlt_idx < len(highlighted_text):
            if original_text[original_idx] == highlighted_text[hlt_idx]:
                ...
                highlighted_text[hlt_idx : hlt_idx + len(self.highlight_token)]
    ...
    def highlight_paragraph(
        self, query_state, para_state, para_words, original_paragraph
    ) -> List[Tuple[int]]:
        ...
        sim_matrix = self.similarity_matrix(vector1=query_state, vector2=para_state)
    ...
    def highlight_paragraphs(
        self, query: str, paragraphs: List[str]
    ) -> List[List[Tuple[int]]]:
