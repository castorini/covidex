...
async def get_search(request: Request, query: str):
    ...
    for base_docid, doc_results in grouped_results.items():
        ...
        for (hit, _) in doc_results:
            paragraph_number = (
                int(hit.docid.split(".")[-1]) if hit.docid != base_docid else -1
            )
        ...
        article = build_article(top_hit, top_score, paragraphs, highlighted_abstract)
    ...
    if settings.highlight:
        ...
        for result in ranked_results:
            ...
            result.highlights = all_highlights[
                highlight_idx : highlight_idx + num_paragraphs
            ]
